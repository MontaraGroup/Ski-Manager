        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-8 items-center">
            <div class="lg:col-span-3">
                ...
                <p class="text-lg text-base-content/70 mb-8 max-w-lg leading-relaxed">
                    Start with an empty mountain and up to <?= currency(1000000) ?> in cash. Build lifts, hire staff, manage snowmaking, and survive <?= getSeasonLength() ?> days without going bankrupt—hire the wrong staff, skip the snow machines, ignore the government, and you're done by Day 10.
                </p>
                <div class="flex gap-3 flex-wrap mb-6">
                    <?php if (!auth()->loggedIn()) : ?>
                    <a href="/register" class="btn btn-primary btn-lg gap-2 shadow-lg hover:scale-105 transition-all"><i class="fa-solid fa-play"></i> Play Free - Takes 30 Seconds</a>
                    <?php else : ?>
                    <a href="/dashboard" class="btn btn-primary btn-lg gap-2 shadow-lg hover:scale-105 transition-all"><i class="fa-solid fa-gauge-high"></i> Go to Your Resort</a>
                    <?php endif ?>
                </div>
                ...
                <div class="card bg-base-100 shadow-xl border border-base-300">
                    <div class="card-body p-5">
                        <div class="flex items-center gap-2 mb-3">
                            ...
                            <span class="text-xs font-semibold text-success">Live Mountain Stats · Day <?= $gameDay ?></span>
                        </div>
                        <?php if ($weather) : ?>
                        <div class="flex items-center gap-3 p-3 bg-base-200/70 rounded-lg mb-3">
                            <i class="fa-solid fa-<?= $weather['temp'] <= -5 ? 'snowflake text-info' : 'cloud-sun text-warning' ?> text-2xl"></i>
                            <div>
                                <div class="font-bold text-sm"><?= temp((int)$weather['temp']) ?> · <?= $weather['condition_name'] ?></div>
                                <div class="text-xs text-base-content/60">Today on the mountain</div>
                            </div>
                        </div>
                        <?php endif ?>
                        <?php if ($topPlayer) : ?>
                        <div class="flex items-center gap-3 p-3 bg-base-200/70 rounded-lg mb-3">
                            ...
                            <div>
                                ...
                                <div class="text-xs text-base-content/60">Richest resort - <?= currency((int) $topPlayer['cash']) ?></div>
                            </div>
                        </div>
                        <?php endif ?>
                        <?php if ($recentPlayer) : ?>
                        <div class="flex items-center gap-3 p-3 bg-base-200/70 rounded-lg">
                            <i class="fa-solid fa-user-plus text-primary text-xl"></i>
                            <div>
                                <div class="font-bold text-sm"><?= esc($recentPlayer['username']) ?></div>
                                <div class="text-xs text-base-content/60">Just joined <?= timeAgo($recentPlayer['created_at']) ?></div>
                            </div>
                        </div>
                        <?php endif ?>
                    </div>
...
<section class="py-12 px-4 bg-base-100">
    <div class="max-w-5xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="card bg-base-200/50 hover:bg-base-200 transition-all border border-base-300">
                <div class="card-body text-center">
                    <div class="text-4xl mb-3">🏔️</div>
                    <h3 class="font-bold mb-2">Opening new slopes</h3>
                    <p class="text-sm text-base-content/70">Drawing runs on an interactive map, choosing difficulty ratings, and watching visitors pour in. <?= number_format($totalSlopes) ?> slopes built so far.</p>
                </div>
            </div>
            <div class="card bg-base-200/50 hover:bg-base-200 transition-all border border-base-300">
                <div class="card-body text-center">
                    ...
                    <p class="text-sm text-base-content/70">Hotels, restaurants, parking fees, ticket sales. The top player has <?= $topPlayer ? currency((int) $topPlayer['cash']) : currency(500000) ?>. Can you beat that?</p>
                </div>
            </div>
            <div class="card bg-base-200/50 hover:bg-base-200 transition-all border border-base-300">
                <div class="card-body text-center">
                    ...
                    <p class="text-sm text-base-content/70">Today it's <?= $weather ? temp((int)$weather['temp']) . ' and ' . strtolower($weather['condition_name']) : 'cold' ?>. Some are turning on snow machines. Others are panicking.</p>
                </div>
            </div>
        </div>
    </div>
...
<section class="py-12 px-4 bg-base-100">
    <div class="max-w-5xl mx-auto">
        <div class="text-center mb-8">
            ...
            <p class="text-base-content/70 max-w-lg mx-auto">Each season unlocks new terrain on Park City Mountain. Compete, build, and expand across the mountain.</p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="card bg-gradient-to-br from-primary/20 to-info/20 border-2 border-primary shadow-md">
                <div class="card-body p-5 text-center">
                    ...
                    <p class="text-xs text-base-content/70 mb-3">The first sector opens with beginner and intermediate terrain. Build your first lifts, hire staff, and establish your resort. <?= getSeasonLength() ?> days to prove yourself.</p>
                    ...
                    <div class="mt-3">
                        ...
                        <div class="text-xs text-base-content/50 mt-1">Day <?= getSeasonDay() ?>/<?= getSeasonLength() ?></div>
                    </div>
                </div>
            </div>
            <div class="card bg-base-200/50 border border-base-300">
                <div class="card-body p-5 text-center">
                    ...
                    <p class="text-xs text-base-content/70 mb-3">The mountain expands. New advanced terrain unlocks with steeper slopes, longer lifts, and higher-altitude challenges. Your Season 1 progress carries over.</p>
            ...
            <div class="card bg-base-200/50 border border-base-300">
                <div class="card-body p-5 text-center">
                    ...
                    <p class="text-xs text-base-content/70 mb-3">The full mountain opens. Expert terrain, backcountry access, and the ultimate leaderboard showdown. Only the best resort managers survive.</p>
                    <div class="flex justify-center gap-3 text-xs text-base-content/50">
    ...
    <div class="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-base-content/70">After Park City, where do we go? Help choose the next resort.</p>
            </div>
        </div>
...
<section class="py-16 px-4 bg-base-200 border-t border-base-300">
    <div class="max-w-5xl mx-auto">
        <div class="text-center mb-8">
            ...
            <p class="text-base-content/70 max-w-lg mx-auto">A deep ski resort tycoon simulator where every system is connected. Hire too many staff? Your expenses spike. Skip insurance? One accident costs everything.</p>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            <div class="card bg-base-100 shadow-sm border border-base-300 hover:shadow-md transition-all"><div class="card-body p-4 items-center text-center"><div class="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center mb-2"><i class="fa-solid fa-map text-primary text-xl"></i></div><div class="text-sm font-bold">Trail Map</div><div class="text-xs text-base-content/60">Draw and build your resort</div></div></div>
            <div class="card bg-base-100 shadow-sm border border-base-300 hover:shadow-md transition-all"><div class="card-body p-4 items-center text-center"><div class="w-12 h-12 rounded-xl bg-warning/10 flex items-center justify-center mb-2"><i class="fa-solid fa-users text-warning text-xl"></i></div><div class="text-sm font-bold">10 Staff Roles</div><div class="text-xs text-base-content/60">Patrol, groomers, chefs...</div></div></div>
            <div class="card bg-base-100 shadow-sm border border-base-300 hover:shadow-md transition-all"><div class="card-body p-4 items-center text-center"><div class="w-12 h-12 rounded-xl bg-info/10 flex items-center justify-center mb-2"><i class="fa-solid fa-cloud-sun text-info text-xl"></i></div><div class="text-sm font-bold">Dynamic Weather</div><div class="text-xs text-base-content/60">Changes every hour</div></div></div>
            <div class="card bg-base-100 shadow-sm border border-base-300 hover:shadow-md transition-all"><div class="card-body p-4 items-center text-center"><div class="w-12 h-12 rounded-xl bg-success/10 flex items-center justify-center mb-2"><i class="fa-solid fa-coins text-success text-xl"></i></div><div class="text-sm font-bold">Economy Simulation</div><div class="text-xs text-base-content/60">Loans, insurance, regulations</div></div></div>
            <div class="card bg-base-100 shadow-sm border border-base-300 hover:shadow-md transition-all"><div class="card-body p-4 items-center text-center"><div class="w-12 h-12 rounded-xl bg-error/10 flex items-center justify-center mb-2"><i class="fa-solid fa-snowflake text-error text-xl"></i></div><div class="text-sm font-bold">Real Brands</div><div class="text-xs text-base-content/60">PistenBully, TechnoAlpin</div></div></div>
            <div class="card bg-base-100 shadow-sm border border-base-300 hover:shadow-md transition-all"><div class="card-body p-4 items-center text-center"><div class="w-12 h-12 rounded-xl bg-secondary/10 flex items-center justify-center mb-2"><i class="fa-solid fa-person-snowboarding text-secondary text-xl"></i></div><div class="text-sm font-bold">Terrain Parks</div><div class="text-xs text-base-content/60">Halfpipes, rail gardens</div></div></div>
            <div class="card bg-base-100 shadow-sm border border-base-300 hover:shadow-md transition-all"><div class="card-body p-4 items-center text-center"><div class="w-12 h-12 rounded-xl bg-warning/10 flex items-center justify-center mb-2"><i class="fa-solid fa-trophy text-warning text-xl"></i></div><div class="text-sm font-bold">Leaderboard</div><div class="text-xs text-base-content/60">Compete globally</div></div></div>
            <div class="card bg-base-100 shadow-sm border border-base-300 hover:shadow-md transition-all"><div class="card-body p-4 items-center text-center"><div class="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center mb-2"><i class="fa-solid fa-bolt text-primary text-xl"></i></div><div class="text-sm font-bold">Energy & Water</div><div class="text-xs text-base-content/60">Manage your resources</div></div></div>
            <div class="card bg-base-100 shadow-sm border border-base-300 hover:shadow-md transition-all"><div class="card-body p-4 items-center text-center"><div class="w-12 h-12 rounded-xl bg-accent/10 flex items-center justify-center mb-2"><i class="fa-solid fa-building-columns text-accent text-xl"></i></div><div class="text-sm font-bold">Compliance</div><div class="text-xs text-base-content/60">Regulations and insurance</div></div></div>
            <div class="card bg-base-100 shadow-sm border border-base-300 hover:shadow-md transition-all"><div class="card-body p-4 items-center text-center"><div class="w-12 h-12 rounded-xl bg-error/10 flex items-center justify-center mb-2"><i class="fa-solid fa-gauge text-error text-xl"></i></div><div class="text-sm font-bold">3 Difficulty Modes</div><div class="text-xs text-base-content/60">Easy, Standard, Hard</div></div></div>
            <div class="card bg-base-100 shadow-sm border border-base-300 hover:shadow-md transition-all"><div class="card-body p-4 items-center text-center"><div class="w-12 h-12 rounded-xl bg-warning/10 flex items-center justify-center mb-2"><i class="fa-solid fa-star text-warning text-xl"></i></div><div class="text-sm font-bold">Achievements</div><div class="text-xs text-base-content/60">Unlock features as you grow</div></div></div>
            <div class="card bg-base-100 shadow-sm border border-base-300 hover:shadow-md transition-all"><div class="card-body p-4 items-center text-center"><div class="w-12 h-12 rounded-xl bg-info/10 flex items-center justify-center mb-2"><i class="fa-solid fa-hotel text-info text-xl"></i></div><div class="text-sm font-bold">Hotels & Dining</div><div class="text-xs text-base-content/60">Lodges, restaurants, rentals</div></div></div>
        </div>
    </div>
</section>

<?php if (!auth()->loggedIn()) : ?>
<section class="py-10 px-4 bg-base-100">
    <div class="max-w-xl mx-auto text-center">
        <h2 class="text-xl font-bold mb-2">Ready to manage your own resort?</h2>
        <p class="text-sm text-base-content/70 mb-4">Free to play. No downloads. Start building in 30 seconds.</p>
...
<section class="py-12 px-4 bg-base-200 border-t border-base-300">
    <div class="max-w-3xl mx-auto">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
            <div class="card bg-base-100 border border-base-300"><div class="card-body p-4"><i class="fa-solid fa-heart text-error text-2xl mb-2"></i><h3 class="font-bold">100% Free</h3><p class="text-xs text-base-content/70">No paywalls, no pay-to-win. Earn everything by playing.</p></div></div>
            <div class="card bg-base-100 border border-base-300"><div class="card-body p-4"><i class="fa-solid fa-globe text-info text-2xl mb-2"></i><h3 class="font-bold">Instant Play</h3><p class="text-xs text-base-content/70">Browser-based. No downloads. Works on any device.</p></div></div>
            <div class="card bg-base-100 border border-base-300"><div class="card-body p-4"><i class="fa-solid fa-code-branch text-success text-2xl mb-2"></i><h3 class="font-bold">Open Source</h3><p class="text-xs text-base-content/70">Built in the open on <a href="https://github.com/MontaraGroup/Ski-Manager" target="_blank" rel="noopener noreferrer" class="link link-primary">GitHub</a>.</p></div></div>
        </div>
    </div>
</section>
