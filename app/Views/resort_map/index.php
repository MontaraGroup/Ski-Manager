...
(function(){
    ...
    function populateList(type){
        ...
        avail.forEach(function(seg){
            ...
            var c=segColor(seg);
            btn.innerHTML='<span style="width:8px;height:8px;border-radius:50%;background:'+c+';flex-shrink:0"></span> <span class="truncate">'+(seg.name||'Unnamed')+'</span><span class="ml-auto text-base-content/40 text-xs">'+Math.round(seg.length_meters||0)+'m</span>';
    ...
    function selectSegment(seg){
        ...
        document.getElementById('selSegLength').textContent=Math.round(seg.length_meters||0)+' m';
    ...
    function finishDraw(){
        ...
        var name=document.getElementById('drawName').value||'Unnamed';
        ...
        for(var i=1;i<drawPoints.length;i++){
            var a=L.latLng(drawPoints[i-1][0],drawPoints[i-1][1]),b=L.latLng(drawPoints[i][0],drawPoints[i][1]);
        ...
        postJSON('/map/segment',{type:(drawType==="slope"?(document.getElementById("drawSlopeType").value||"downhill"):drawType),name:name,points:drawPoints,length_meters:Math.round(totalM),difficulty:diff,sector:sector},function(res){
            if(res.success) location.reload(); else alert(res.error||'Save failed');
        });
    }
